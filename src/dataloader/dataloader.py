...
class SpatioTemporalSequenceDatamoduleJSON(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.seed = seed
        # Volumes are expected to have been preprocessed offline. Keeping the
        # online pipeline empty also matches the registration pretraining
        # project, so pretrained weights see the same input representation.
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        dataset = SpatioTemporalDataset(self.data_val, self.data_train)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        dataset = SpatioTemporalDatasetValidation(self.data_val)
        return torch.utils.data.DataLoader(
            dataset=dataset,
            batch_size=1,
            num_workers=1,             # ← Single worker
