...
def generate_grid3d_tensor(shape: Sequence[int]) -> Tensor:
    ...
    The channel order is ``(z, y, x)`` to match
    ...
    """
    x = torch.linspace(-1.0, 1.0, shape[0])
    y = torch.linspace(-1.0, 1.0, shape[1])
    z = torch.linspace(-1.0, 1.0, shape[2])
...
def warp(image: Tensor, flow: Tensor, mode: str = 'bilinear') -> Tensor:
    ...
    Out-of-bounds coordinates are handled by replicating the closest border
    value, avoiding both black artefacts and anatomically implausible mirror
    content.
