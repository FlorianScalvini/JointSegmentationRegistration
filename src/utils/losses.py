...
class NonDetJacobianPenalty(nn.Module):
    """Smooth Jacobian-determinant barrier for folding prevention.

    For every voxel, the penalty is

    ``temperature * softplus((epsilon - det(J)) / temperature)``,

    with ``epsilon = 0.05`` and ``temperature = 0.01``. The returned loss is
    the mean of these voxelwise penalties. This is a smooth approximation of
    ``max(0, epsilon - det(J))``: determinants well above ``epsilon`` incur
    almost no penalty, while small or non-positive determinants are
    increasingly penalised. Minimising the loss therefore discourages both
    near-singular and topology-breaking deformations.
    """
    ...
    def forward(
        ...
        """Compute the smooth Jacobian barrier for a displacement field.
        ...
        Returns:
            Scalar tensor containing the mean smooth barrier penalty across
            all voxels and batch elements.
        """
