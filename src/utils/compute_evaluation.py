...
def read_parcellations(path: str, num_classes: int) -> torch.Tensor:
    ...
    if label.data.shape[0] != 1:
        ...
        raw_labels = label.data.long()
        merged_labels = torch.where(
            raw_labels <= 1, torch.zeros_like(raw_labels), raw_labels - 1
        )
        label = tio.LabelMap(tensor=merged_labels, affine=affine)
...
def compute_evaluation(
    ...
    val_json_path = config['root_dir'] + config['val_json']
    labels_config = config['root_dir'] + config['label_metadata']
    ...
    preds_segmentation = glob.glob(os.path.join(pred_path, "segmentations", "*.nii*"))
    preds_parcellations = glob.glob(os.path.join(pred_path, "registration_parcellations", "*.nii*"))
    preds_images = glob.glob(os.path.join(pred_path, "registration_images","*.nii*"))
    ...
    data_json = json.load(open(val_json_path))


    lst_data_gt = []
    for i in data_json['subjects']:
        for j in i["sessions"]:
            lst_data_gt.append((j['age'], (root_dir +  j['image']),(root_dir + j['segmentation'])))
    ...
    """
    if not compute_gi:
        for data in lst_data_gt:
            seg = data[2]
            img = tio.ScalarImage(seg)
            output_path = seg.replace('nii.gz', 'vtk')
            if img.data.shape[0] != 1:
                ...
                seg = './temp/img.nii.gz'
            convert_nifti_labels_union_to_vtk(seg, cortex_labels, output_path)
        if os.path.exists("./temp/"):
            shutil.rmtree("./temp/")
        header = ["time", "GI", "Normalized_GI"]
        path_csv = os.path.join(root_dir, f"gi.csv")
        initial_smooth_mesh_path = initial_paths[2].replace('.nii.gz', '.vtk')
        vtk_path_gt = initial_smooth_mesh_path.split("vtk")[0]
        if os.path.exists("./temp/"):
            shutil.rmtree("./temp/")
        initial_smooth_mesh = meshio.read(lst_data_gt[0][2].replace('.nii.gz', '.vtk'))
        with open(path_csv, mode="w", newline="") as f:
            writer = csv.writer(f, delimiter=" ")
            writer.writerow(header)
            for data in lst_data_gt:
                seg = data[2]
                vtk_file = seg.replace('nii.gz', 'vtk')
                print(vtk_file)
                folded_mesh = meshio.read(vtk_file)
    ...
    if args.eval_mode in ["both", "registration"]:
        if create_img_slice :
            save_path = os.path.join(parentPath, "image_slices")
            os.makedirs(save_path, exist_ok=True)
            for i in range(len(preds_images)):
                output_path = os.path.join(save_path, f"slice_reg_{i}.png")
        ...
        if not compute_dice:
            ...
            processed_rows = 0
            path_csv = os.path.join(parentPath, f"dice_reg.csv")
            save_path_error = os.path.join(parentPath, "error_seg_slices_reg")
            save_path_seg = os.path.join(parentPath, "seg_slices_reg")
            ...
            with open(path_csv, mode="w", newline="") as f:
                ...
                for i in range(len(preds_parcellations)):
                    ...
                    # rst shape: (B=1, C, ...)
                    per_label = rst[0]  # (C,)
                    mdice = float(np.mean(per_label.numpy()))
                    cortex_values = []
                    ...
                    output_path = os.path.join(save_path_error, f"slice_reg_{i}.png")
                    error_map_img.save(output_path)
                    output_path = os.path.join(save_path_seg, f"slice_reg_{i}.png")
        ...
        if compute_flow:
            ...
            with open(path_csv, mode="w", newline="") as f:
                ...
                for i in range(len(preds_flows)):
                    ...
                    composite.save(os.path.join(image_save_path, f"slice_reg_{i}.png"))
                    ...
                    grid_plot.save(os.path.join(grid_save_path, f"slice_reg_{i}.png"))
            ...
            if n > 0:
                ...
                plt.savefig(os.path.join(image_save_path, f'jacobian_all_reg.png'), bbox_inches='tight')

        # Step 1: Create vtk files for cortex labels
        if not create_vtk:
            vtk_path = os.path.join(pred_path, "registration_vtk")
        ...
        if not compute_gi:
            header = ["time", "GI", "Normalized_GI"]
            path_csv = os.path.join(parentPath, f"gi_reg.csv")
            initial_smooth_mesh_path = initial_paths[2].replace('.nii.gz', '.vtk')
            if os.path.exists("./temp/"):
                shutil.rmtree("./temp/")
            ...
            with open(path_csv, mode="w", newline="") as f:
                writer = csv.writer(f, delimiter=" ")
                writer.writerow(header)
                vtk_files = glob.glob(os.path.join(pred_path, "registration_vtk", "*.vtk"))
                vtk_files.sort()
                for i in range(len(vtk_files)):
                    ...
                    row = [lst_data_gt[i][0]]
                    row.append(GI)
                    data = pd.read_csv(root_dir + "gi.csv", sep=" ")
                    gi_gt = data[data['time'] == lst_data_gt[i][0]].values[0][1]
                    row.append(GI / gi_gt)
                    writer.writerow(row)
            # Step 2 : Compute the dice score for all the parcellations
    if args.eval_mode in ["both", "segmentation"]:
        if not compute_dice:
            header = ["time", "mDice", "cortex"] + labelsNames
            dice = DiceMetric(include_background=True, reduction="none", ignore_empty=False)
            processed_rows = 0
            path_csv = os.path.join(parentPath, f"dice_seg.csv")
            ...
            with open(path_csv, mode="w", newline="") as f:
                writer = csv.writer(f, delimiter=" ")
                writer.writerow(header)
                for i in range(len(preds_segmentation)):
                    print(preds_segmentation[i])
                    pred_1h = read_parcellations(str(preds_segmentation[i]), num_classes=num_classes)
                    print(lst_data_gt[i][2])
                    gt_1h = read_parcellations(lst_data_gt[i][2], num_classes=num_classes)
                    rst = dice(y_pred=pred_1h.unsqueeze(0), y=gt_1h.unsqueeze(0))
                    dice.reset()
                    ...
                    img = tio.ScalarImage(preds_images[i])
                    mask = tio.ScalarImage(preds_segmentation[i]).data
                    ...
                    slice_seg = segmentation_map_slice_with_color_map(slice_seg, rotate=rotate)
                    error_map_img = error_map_slice_with_color_map(img, error_map, alpha=0.8, rotate=rotate)
                    output_path = os.path.join(save_path_error, f"slice_seg{i}.png")
                    error_map_img.save(output_path)
                    output_path = os.path.join(save_path_seg, f"slice_seg{i}.png")
                    slice_seg.save(output_path)
                    print(output_path)
        if not create_vtk:
            vtk_path = os.path.join(pred_path, "vtk_segmentations")
            os.makedirs(vtk_path, exist_ok=True)
            for p_seg in preds_segmentation:
                img = tio.ScalarImage(p_seg)
                output_path = p_seg.replace('segmentations', 'vtk_segmentations').replace('nii.gz', 'vtk')
                if img.data.shape[0] != 1:
                    os.makedirs("./temp/", exist_ok=True)
                    img_tensor = torch.argmax(img.data, dim=0).unsqueeze(0)
                    img = tio.LabelMap(tensor=img_tensor.int(), affine=img.affine)
        ...
        if not compute_gi:
            ...
            initial_smooth_mesh_path = initial_paths[2].replace('segmentations', 'vtk_segmentations').replace('.nii.gz', '.vtk')
            if os.path.exists("./temp/"):
                shutil.rmtree("./temp/")
            ...
            with open(path_csv, mode="w", newline="") as f:
                writer = csv.writer(f, delimiter=" ")
                writer.writerow(header)
                vtk_files = glob.glob(os.path.join(pred_path, "vtk_segmentations", "*.vtk"))
                vtk_files.sort()
                for i in range(len(vtk_files)):
                    print(vtk_files[i])
                    folded_mesh = meshio.read(vtk_files[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_yaml', type=str, help='Path to dataset yaml file', default="/home/florian/VisualStudioProjects/JointSegmentationRegistration/data/babofet.yaml")
    parser.add_argument('--pred', type=str, required=False, help='Predicted segmentation file path', default="//media/florian/Nouveau nom/Result_joint/pretrain")
    parser.add_argument('--eval_mode', type=str, help='Evaluation mode', default="both")
    ...
    print(args)
    compute_evaluation(args.dataset_yaml, args.eval_mode, args.pred, args.dice, args.vtk,  args.flow, args.gi, args.img_slice, args.plane, args.gi_normalized, args.rotate)
