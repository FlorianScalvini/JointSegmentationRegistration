...
class SpatioTemporalSequenceDatamoduleJSON(pl.LightningDataModule):
    ...
    def __init__(
        ...
        if augmentation:
            self.transform_aug = tio.transforms.Compose([
                ...
                tio.transforms.RandomNoise(mean=0, std=0.2),
                tio.transforms.RandomBiasField(coefficients=0.2),
                tio.transforms.RandomGamma(log_gamma=(-0.2, 0.2)),
                tio.transforms.RandomBlur(std=(0, 0.3))
            ])
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        dataset = SpatioTemporalDataset(self.data_train, self.transform, transform_aug=self.transform_aug)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        dataset = SpatioTemporalDatasetValidation(self.data_val, self.transform, None)
        return torch.utils.data.DataLoader(
            dataset=dataset,
            batch_size=1,
            num_workers=1,             # ← Single worker
